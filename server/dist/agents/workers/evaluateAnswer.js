...
const CriterionEvaluationSchema = zod_1.z.object({
    criterion: zod_1.z.string().describe("평가 기준의 이름 (예: '정확성 및 기술적 깊이')"),
    score: zod_1.z.number().min(1).max(5).describe("해당 기준에 대한 1(미흡)에서 5(우수) 사이의 점수"),
    reasoning: zod_1.z.string().describe("점수에 대한 간결한 근거. 답변 내용에서 직접적인 증거를 인용해야 함."),
});
exports.EvaluationResultSchema = zod_1.z.object({
    overall_score: zod_1.z.number().describe("모든 기준의 가중 평균 점수."),
    evaluations: zod_1.z.array(CriterionEvaluationSchema).describe("각 기준별 평가 결과 목록."),
    is_sufficient: zod_1.z.boolean().describe("답변이 최소 요구 수준(예: 전체 점수 3.0 이상)을 충족했는지 여부."),
});
const evaluationModel = new google_genai_1.ChatGoogleGenerativeAI({
    model: "gemini-2.0-flash",
    temperature: 0,
    apiKey: process.env.GOOGLE_API_KEY,
}).withStructuredOutput(exports.EvaluationResultSchema);
