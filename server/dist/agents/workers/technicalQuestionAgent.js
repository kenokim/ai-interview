...
exports.technicalQuestionAgent = technicalQuestionAgent;
...
async function technicalQuestionAgent(state) {
...
- 이미 질문한 주제: ${task.questions_asked.map(q => q.category).join(", ") || "없음"}`;
    try {
        ...
        const availableQuestions = task.question_pool.filter(q => !task.questions_asked.some(asked => asked.id === q.id));
        if (availableQuestions.length === 0) {
            return {
                messages: [new messages_1.AIMessage("모든 질문이 완료되었습니다. 면접을 마무리하겠습니다.")],
                task: {
        ...
        return {
            messages: [new messages_1.AIMessage(result.content)],
            task: {
        ...
        return {
            messages: [new messages_1.AIMessage("죄송합니다. 질문을 생성하는 중 오류가 발생했습니다.")],
            guardrails: {
                is_safe: false,
